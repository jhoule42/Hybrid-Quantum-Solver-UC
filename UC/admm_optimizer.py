...
class ADMMParameters:
    ...
    def __init__(
        ...
        self.p = p
        self.gammas0 = gammas0, # should maybe change here the values of gamma and beta???
        self.beta0 = betas0
...
class ADMMOptimizer(OptimizationAlgorithm):
    ...
    **References:**
    ...
    VALID_QUBO_TYPES = {"classical", "quantum_basic", "quantum_advance"}
    ...
    def __init__(
        ...
        backend : QiskitRuntimeService = None, # need to change the object type
        ...
        Args:
            qubo_type (str): Specifies the type of QUBO solver. 
                ...
                - "qaoa_basic": Uses the provided `qubo_optimizer` (must be a QAOA instance).
                - "quantum_basic": Uses the provided `qubo_optimizer` (for other quantum solvers).

            qubo_optimizer (Optional[OptimizationAlgorithm]): 
                An instance of `OptimizationAlgorithm` for QUBO problems.
                Required if `qubo_type` is "qaoa_basic" or "quantum_basic".
        ...
        while (iteration < self._params.maxiter and residual > self._params.tol) and (
            ...
            if self._state.step1_absolute_indices:
                ...
                if self._qubo_type == 'qaoa_advance':
                    ...
                    print('QAOA Advance mode')
                    
                    # linear initialization of gammas & betas parameters     
                    x0_params = np.empty(2 * self._params.p, dtype=float)
                    x0_params[0::2] = np.linspace(0, 2*np.pi,  self._params.p) # gammas
                    x0_params[1::2] = np.linspace(np.pi, 0,  self._params.p) # betas
                    ...
                    if self._hardware_execution:
                        ...
                        # Change the transpiling for the fakeProvider()
                        isa_circuit = pm.run(circuit) # start transpiling
                        # print('Transpiling done.')

                        self._state.counts = self._update_x0_qaoa_hardware(op_ising, isa_circuit, x0_params,
                                                                           backend, pm)
                    ...
                    # print(f"Counts: {self._state.counts}")
                    print(f"bitstring: {max_bitstring} - type: {type(max_bitstring)}")

                    # Not sure why this is usefull !!!
                    x0_all_binaries = np.array([int(bit) for bit in max_bitstring])
                    # x0_qubo = np.asarray(max_bitstring)
                    print(f"relative_indices: {self._state.step1_relative_indices}")
                    # print(f"x0_qubo: {x0_qubo} - type: {type(x0_qubo)}")
                    print(f"x0_all_binaries: {x0_all_binaries} - type: {type(x0_all_binaries)}")
    ...
    def _update_x0_qaoa_hardware(self, cost_hamiltonian, isa_circuit, x0_params, backend, pass_manager):
        ...
        # Code to use to run on real hardware
        ...
        # print(f"Final distribution int: {final_distribution_int}")
    ...
    def _update_x0(self, op1 : QuadraticProgram) -> np.ndarray:
        ...
        # Still need to understand what x0_all_binaries does!
        print(f"1) x0_qubo: {x0_qubo}", type(x0_qubo))
        print(f"2) x0_all_binaries: {x0_all_binaries}",  type(x0_all_binaries))
